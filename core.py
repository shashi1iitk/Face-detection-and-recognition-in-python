...
#This file contains all common functions that are called in *Tester.py file

#creating dictionary containing levels for each name/people
...
def faceDetection(test_img):
    ...
    faces=face_haar_cascade.detectMultiScale(gray_img,scaleFactor=1.32,minNeighbors=5)#detectMultiScale returns rectangle coordinates
...
def draw_rect(test_img,face):
    (x,y,w,h)=face
    cv2.rectangle(test_img,(x,y),(x+w,y+h),(0,100,0),thickness=2)

#Below function writes name of person for detected label
def put_text(test_img,text,x,y):
    cv2.putText(test_img,text,(x,y),cv2.FONT_HERSHEY_COMPLEX,1,(255,0,0),1)
